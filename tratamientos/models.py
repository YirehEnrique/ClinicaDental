...
class PlanTratamiento(models.Model):
    ...
    precio_estimado = models.DecimalField(max_digits=10, decimal_places=2, default=0.00)
    sesiones_estimadas = models.IntegerField(null= True, default=0) 
    sesiones_realizadas = models.IntegerField(null=True, default=0)
...
class ItemSesion(models.Model):
    #plan_item=models.ForeignKey(PlanItems, default=1,on_delete=models.CASCADE)
    nombre_sesion = models.CharField(max_length=100, default='')
    #notas=models.TextField(blank=True, null=True)
    plan_tratamiento = models.ForeignKey('PlanTratamiento', null=True, blank=True, on_delete=models.CASCADE, related_name="sesiones")
    #cita=models.ForeignKey(Cita, null=True, blank=True, on_delete=models.SET_NULL)
    creada=models.DateTimeField(auto_now_add=True)
    actualizada=models.DateTimeField(auto_now=True, blank=True, null=True)

    def __str__(self):
        return f'{self.id}: {self.plan_tratamiento}'